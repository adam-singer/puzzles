/*
* Rob DiMarco's solution:
* http://robdimarco.com/code/2009/10/21/facebook-jobs-puzzles-sophie.html
* This solution passed the Facebook Puzzlebot with a time of 149.516 ms
* on its longest test case.
*/
...
public class Sophie {
    ...
    /*
    * Floyd-Warshall algorithm for all-pairs shortest paths. This
    ...
    private static void visit(int node, double ev, double probability) {
        ...
        for (int i = 1; i < size; i++) {
            if (!visited[i]) {
                ...
                double x = ev + (probability*adjMatrix[node][i]);
                if (x < best) {
                    visit(i, x, probability);
                }
            }
        }
        visited[node] = false;
